...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const scriptUrl = process.env.GOOGLE_SHEET_SCRIPT_URL_2;
    if (scriptUrl) {
      try {
        const sheetRes = await fetch(scriptUrl, {
          method: "POST",
          headers: { "Content-Type": "text/plain" },
          body: JSON.stringify({ formType: "contact2", fullName, phoneNumber, email }),
          redirect: "follow",
        });
        if (!sheetRes.ok) {
      ...
      console.warn("GOOGLE_SHEET_SCRIPT_URL_2 not set, skipping Sheets.");
    }
