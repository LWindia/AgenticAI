  ...
  return (
    <header className="fixed top-0 z-50 w-full bg-black border-b border-zinc-800">
      <div className="container mx-auto flex items-center justify-between py-4 px-6">
        ...
        <nav className="hidden lg:flex space-x-6">
          <ul className="flex space-x-6">
            {NAV_ITEMS.map((item, index) => (
              <li key={index}>
                <a href={item.href} className="desktop-button-lift text-white hover:text-purple-400 transition-colors duration-200"
                  onClick={(e) => { e.preventDefault(); setDropdownOpen(!isDropdownOpen); }}>
                  {item.text}
                </a>
              </li>
            ))}
            <a href="#timeline" className="desktop-button-lift text-white hover:text-purple-400 transition-colors duration-200">
              AI Warrior Projects
            </a>
          </ul>
        </nav>
        ...
        {/* Mobile Menu Toggle */}
        <button className="desktop-button-lift lg:hidden p-2 text-white hover:text-purple-400 transition-colors duration-200"
        ...
        {/* Desktop CTA */}
        <Link href="#contact" className="desktop-button-lift hidden lg:block bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-2 rounded-lg hover:opacity-90 transition-all duration-300 text-center">
          Enquire Now
        </Link>
      </div>
      ...
      <div
        className={`lg:hidden absolute left-4 right-4 top-full z-10 rounded-lg border border-gray-800 bg-black shadow-lg transition-all duration-200 ease-out ${
      ...
      >
        <div className="p-4">
          <button
            type="button"
            className="desktop-button-lift flex w-full items-center justify-between text-left text-white hover:text-purple-400 transition-colors duration-200"
              ...
              <div className="mb-3 flex gap-2 overflow-x-auto pb-1">
                {roadmapData.map((level) => (
                  <button
                    key={level.level}
                    type="button"
                    className={`desktop-button-lift shrink-0 rounded-full border px-3 py-1 text-xs font-semibold transition-colors ${
          ...
          <Link href="#contact"
            className="desktop-button-lift block w-full mt-6 bg-gradient-to-r from-purple-600 to-pink-600 text-white px-4 py-2 rounded-lg hover:opacity-90 transition-all duration-300 text-center"
      ...
      <div
        ref={dropdownRef}
        className={`absolute left-0 top-full z-20 w-full rounded-xl border border-white/10 bg-zinc-950 shadow-lg shadow-black/40 sm:w-4/5 md:left-40 lg:left-40 lg:shadow-2xl lg:shadow-black/50 transition-all duration-200 ease-out ${
          isDropdownOpen && selectedLevel ? "pointer-events-auto translate-y-0 opacity-100" : "pointer-events-none -translate-y-2 opacity-0"
        }`}
        aria-hidden={!(isDropdownOpen && selectedLevel)}
      >
        {selectedLevel && (
